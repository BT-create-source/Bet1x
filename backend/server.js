...
/**
 * Create a new user record, falling back to the flat-file store on the same terms as
 * getOrCreateUser above. Used by signup, which needs the caller's actual email/password rather
 * than the placeholder values getOrCreateUser's own auto-create path fills in.
 */
async function createUserRecord({ username, email, password, wallet_balance }) {
  try {
    return await prisma.user.create({ data: { username, email, password, wallet_balance } });
  } catch (e) {
    if (!jsonFallbackAllowed()) throw e;
    const users = readJsonTable('users');
    const user = {
...
// Auth proxy
app.all('/api/auth.php', authLimiter, requireDatabase, async (req, res, next) => {
  const action = req.query.action || req.body.action || '';
  ...
  try {
    if (action === 'login') {
      ...
      const user = await getOrCreateUser(username, { allowCreate: false });
      if (user && bcrypt.compareSync(password, user.password)) {
        res.json({
          success: true,
      ...
      const existing = await getOrCreateUser(username, { allowCreate: false });
      if (existing) {
        return res.status(400).json({ error: 'Username is already taken.' });
      }
      ...
      if (tokenUser) {
        const user = await getOrCreateUser(tokenUser, { allowCreate: false });
