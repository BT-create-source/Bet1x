...
const firstNames = ['Amit', 'Raj', 'Raju', 'Vikram', 'Sana', 'Pooja', 'Sunil', 'Neha', 'Vijay', 'Rahul', 'Arjun', 'Simran', 'Kunal', 'Deepak', 'Sanjay', 'Preeti'];
const lastNames = ['Kumar', 'Singh', 'Sharma', 'Verma', 'Patel', 'Das', 'Joshi', 'Gupta', 'Mehta', 'Reddy'];
let mockPlayers = [];
...
function generateMockBets() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const amount = Math.floor(Math.random() * 9 + 1) * (Math.random() > 0.5 ? 50 : 10);
    const target = Math.random() > 0.15 ? (Math.random() * 4 + 1.1) : 0; // 0 means crash
    mockPlayers.push({
      name: name,
      amount: amount,
      targetMult: target,
  ...
  document.getElementById('livePlayersCount').textContent = `${mockPlayers.length + 80} playing`;
  renderLiveBets();
}
