<?php
/**
 * logic.php — Football (soccer) betting-style DEMO (LEARNING BUILD)
 *
 * SCOPE NOTE (same as the earlier demos): virtual points only, no real
 * money, no payment gateway, no real teams/leagues/live-data feeds. Uses
 * fictional club names and simulated match results so it's clearly a
 * teaching sandbox, not a live betting product.
 *
 * What it teaches:
 *   1) How bookmaker-style odds are derived from a probability model
 *      (Poisson goal model here) plus a margin/overround
 *   2) Common bet types: 1X2 (match winner), Over/Under 2.5 goals,
 *      Both Teams To Score (BTTS), Correct Score
 *   3) Accumulator (parlay) settlement: all legs must win
 *   4) Provably-fair result generation (server seed committed up front,
 *      revealed after the match is simulated, so it can be verified)
 */
...
class FootballBettingGame
{
    ...
    const MARGIN            = 1.06; // ~6% bookmaker overround baked into odds
    const MAX_GOALS_GRID    = 8;    // probability grid cap (tail folded into edges)
    const NUM_MATCHES       = 4;

    // Fictional clubs only — deliberately not real teams/leagues.
    const TEAM_POOL = [
    ...
    // Deterministic uniform-in-[0,1) derived from a seed + label, so results
    // are reproducible/verifiable from the revealed seed (same idea as the
    // crash game and mines demos).
    private function uniformFromSeed(string $seed, string $label): float
    {
        $hex = substr(hash('sha256', $seed . ':' . $label), 0, 13); // 52 bits
    ...
    private function samplePoisson(float $lambda, float $u): int
    {
        ...
        return self::MAX_GOALS_GRID; // tail fallback
    }

    // Builds the full joint scoreline probability grid for a match.
    ...
    private function generateFixtures(): void
    {
        ...
        for ($i = 0; $i < self::NUM_MATCHES; $i++) {
            ...
            // Randomised attacking strengths -> Poisson goal expectancies,
            // with a small home-advantage bump.
            ...
            $matches[] = [
                ...
                'home'        => $home,
                'away'        => $away,
                'lambda_home' => $lambdaHome, // server-side only, never sent to client
    ...
    // ---- Bet placement ----------------------------------------------------------
    // $legs = [ ['match_id'=>1, 'bet_type'=>'match_winner', 'selection'=>'home'], ... ]
    // A single-leg bet is just a 1-leg accumulator.
    ...
    private function oddsForSelection(array $match, string $betType, string $selection): ?float
    {
        switch ($betType) {
            case 'match_winner':
                return $match['odds'][$selection] ?? null; // home|draw|away
            case 'over_under':
                return $match['odds'][$selection] ?? null; // over25|under25
            case 'btts':
                return $match['odds'][$selection] ?? null; // btts_yes|btts_no
            case 'correct_score':
                return $match['odds']['correct_score'][$selection] ?? null; // e.g. "2-1" or "other"
            default:
                return null;
        }
    }

    // ---- Simulate a match + settle all bets referencing it ---------------------
    ...
    private function settleBetsForMatch(int $matchId, int $h, int $a): void
    {
        foreach ($_SESSION['fb_bets'] as &$bet) {
            // Grade any still-pending leg for this match regardless of the
            // bet's overall status, so the UI always shows a leg's true
            // win/lose result once its match is finished — even if the bet
            // was already marked lost because of an earlier leg.
            ...
            if ($bet['status'] === 'lost' || $bet['status'] === 'won') continue; // already decided, never reopen
