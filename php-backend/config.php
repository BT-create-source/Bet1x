...
//
// PHONE_VERIFICATION_REQUIRED is the master switch. With it false nothing changes: signup ignores
// phone entirely and no SMS is ever sent, so the feature can be deployed dark and turned on once the
// SMS account is confirmed working.
$PHONE_VERIFICATION_REQUIRED = env_bool('PHONE_VERIFICATION_REQUIRED', false);

// The Fast2SMS key. Server-side only — it is never sent to the browser, because anyone holding it
// can send SMS billed to this account.
$FAST2SMS_API_KEY = (string) env_get('FAST2SMS_API_KEY', '');

// Which Fast2SMS route the account is provisioned for. India requires DLT registration for
// transactional SMS, and which routes a given key may use depends on that registration, so this is
// configuration rather than a hardcoded choice:
//   'otp' — the OTP route, message is the code itself via variables_values
//   'dlt' — a DLT-approved template, needs FAST2SMS_MESSAGE_ID and a registered sender id
//   'q'   — the quick/promotional route
$FAST2SMS_ROUTE      = (string) env_get('FAST2SMS_ROUTE', 'otp');
$FAST2SMS_SENDER_ID  = (string) env_get('FAST2SMS_SENDER_ID', '');
$FAST2SMS_MESSAGE_ID = (string) env_get('FAST2SMS_MESSAGE_ID', '');

// A production boot with verification switched on but no key would accept signups it can never send
// a code for, so refuse to start rather than discover it from customers who cannot register.
if ($IS_PRODUCTION && $PHONE_VERIFICATION_REQUIRED && $FAST2SMS_API_KEY === '') {
    $fatal[] = 'PHONE_VERIFICATION_REQUIRED is on but FAST2SMS_API_KEY is empty. No signup could '
...
// Registration carries no email confirmation and no KYC, and phone verification is off unless
// PHONE_VERIFICATION_REQUIRED is set above — so in the default configuration these are the only
...
$CONFIG = [
    ...
    'OTP_MAX_SENDS_PER_DAY'         => $OTP_MAX_SENDS_PER_DAY,
    'DB'                       => $DB,
