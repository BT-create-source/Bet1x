...
// --- Database ---
// DATABASE_URL keeps the same name and URL shape as the Node build so one variable carries over,
// but the scheme is now mysql://. Discrete DB_* variables are also accepted because cPanel's own
// database wizard hands those out separately.
$DATABASE_URL = (string) env_get('DATABASE_URL', '');
$DB = [
    'host'    => (string) env_get('DB_HOST', '127.0.0.1'),
    'port'    => (int) env_num('DB_PORT', 3306),
