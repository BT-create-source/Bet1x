    ...
    $app->post('/api/db/recent-results', function (Req $req, Res $res) {
        try {
            ...
            q('INSERT INTO `RecentResult` (`room`,`roundNumber`,`number`,`color`,`dotClass`,`size`)
               VALUES (?,?,?,?,?,?)
               ON DUPLICATE KEY UPDATE `number`=VALUES(`number`), `color`=VALUES(`color`),
                                       `dotClass`=VALUES(`dotClass`), `size`=VALUES(`size`)',
                ...
                if ($table === 'users') {
                    ...
                    foreach ($data as $item) {
                        ...
                        q('INSERT INTO `Transaction` (`id`,`user`,`type`,`amount`,`details`,`status`,`timestamp`)
                           VALUES (?,?,?,?,?,?,?)
                           ON DUPLICATE KEY UPDATE `user`=VALUES(`user`), `type`=VALUES(`type`), `amount`=VALUES(`amount`),
                                                   `details`=VALUES(`details`), `status`=VALUES(`status`), `timestamp`=VALUES(`timestamp`)',
                    ...
                    foreach ($data as $item) {
                        ...
                        q('INSERT INTO `Deposit`
                           (`deposit_id`,`order_id`,`username`,`amount`,`utr`,`qr_type`,`custom_qr_data`,`status`,`gateway`,`gateway_id`,`created_at`,`updated_at`)
                           VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
                           ON DUPLICATE KEY UPDATE `order_id`=VALUES(`order_id`), `username`=VALUES(`username`),
                             `amount`=VALUES(`amount`), `utr`=VALUES(`utr`), `qr_type`=VALUES(`qr_type`),
                             `custom_qr_data`=VALUES(`custom_qr_data`), `status`=VALUES(`status`),
                             `gateway`=VALUES(`gateway`), `gateway_id`=VALUES(`gateway_id`), `updated_at`=VALUES(`updated_at`)',
                    ...
                    foreach ($data as $item) {
                        ...
                        q('INSERT INTO `Withdrawal` (`withdrawal_id`,`username`,`amount`,`method`,`details`,`status`,`created_at`,`updated_at`)
                           VALUES (?,?,?,?,?,?,?,?)
                           ON DUPLICATE KEY UPDATE `username`=VALUES(`username`), `amount`=VALUES(`amount`),
                             `method`=VALUES(`method`), `details`=VALUES(`details`), `status`=VALUES(`status`),
