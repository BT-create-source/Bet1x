 ...
 *
 * master_manual is the single switch admin.html's Teen Patti "Bot Mode" toggle controls: false
 * (Auto, the default) means every room's target comes from the periodic shuffle below and the
 * per-room admin inputs are hidden; true (Manual) stops the shuffle entirely and every room's
 * target is whatever an operator last set it to via POST .../admin/room-bots, indefinitely.
 */
